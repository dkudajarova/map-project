...
export type QualityDashboardData = {
  ...
  hail: { eligibleTotal: number; excludedTotal: number; outcomes: DashboardDatum[] }
}
...
export function getQualityDashboardData(): QualityDashboardData {
  ...
  for (const feature of buildings.features) {
    ...
    const assessor = asCount(properties.assessor_record_count) > 0
    const hail = asCount(properties.hail_match_count) > 0
  ...
  const statusIndex = header.indexOf("match_status"); if (statusIndex < 0) throw new Error("Hail match audit is missing match_status")
  const hailCounts = new Map<string, number>()
  for (const row of hailRows.slice(1)) { const status = row[statusIndex]; if (status) hailCounts.set(status, (hailCounts.get(status) ?? 0) + 1) }
  ...
  return {
    ...
    hail: {
      eligibleTotal, excludedTotal: hailCounts.get("excluded") ?? 0,
